        ...
        <div class="row">
            @foreach($favorite_products as $favorite_product)
                <div class="col-md-7 mt-2">
                    <div class="d-inline-flex">
                        ...
                        <div class="container mt-3">
                            ...
                            <h6 class="w-100 samuraimart-favorite-item-text">{{ $favorite_product->price }}</h6>
                        </div>
                    </div>
                </div>
                <div class="col-md-2 d-flex align-items-center justify-content-end">
                    ...
                    <form action="{{ route('favorites.destroy', $favorite_product->id) }}" class="d-none" id="favorites-destroy-form{{ $favorite_product->id }}" method=""POST>
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
                <div class="col-md-3 d-flex align-items-center justify-content-end">
                    <form action="{{ route('carts.store') }}" method="post" class="m-3 align-items-end">
                        ...
                        <button class="btn samuraimart-favorite-add-cart">
                        カートに入れる
                    </button>
                    </from>
                </div>
